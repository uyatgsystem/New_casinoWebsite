...
@Component({
  ...
  styleUrl: './spinner-new.component.scss',
...
export class SpinnerNewComponent {
  ...
  async freeSpin() {
    ...
    // Add animation end event listener to handle the end of spinning
    // const wheelWithSegments = document.querySelector('.wheel-with-segments');
    // if (wheelWithSegments) {
    //   wheelWithSegments.addEventListener(
    //     'animationend',
    //     () => this.handleSpinEnd(),
    //     {
    //       once: true,
  ...
  private handleSpinEnd(): void {
    // Calculate which segment is at the top (winner) - adjust for the direction of rotation
    const rotationModulo = this.wheelRotation % 360;
    const normalizedRotation =
      rotationModulo >= 0 ? rotationModulo : 360 + rotationModulo;
    const winningIndex =
      Math.floor(normalizedRotation / this.segmentAngle) % this.segments.length;
    this.highlightedIndex = winningIndex;

    // Get the winning prize
    const winningPrize = this.segments[winningIndex].prize;
    this.lastWin = winningPrize;

    // Reset spinning states
    ...
    setTimeout(() => {
      Swal.fire({
        ...
        text: `You won ${winningPrize}!`,
        customClass: { popup: 'swal-compact' },
      });

      // Emit spin end event
      this.spinEnd.emit(winningPrize);
    }, 500);
  }

  public startSpinAnimation(): void {
    if (this.isSpinning || this.isWheelSpinning) return;
    ...
    // Calculate the winning segment (random for demo)
    // const winningIndex = Math.floor(Math.random() * this.segments.length);
    const winningIndex = 1;

    // Calculate final rotation (multiple full rotations + position to land on winning segment)
    const baseRotation = 1800; // 5 full rotations (360 * 5)
    const segmentRotation = winningIndex * this.segmentAngle;
    // Add a small random offset to make each spin slightly different
    const randomOffset = Math.random() * this.segmentAngle * 0.5;
    this.wheelRotation = baseRotation + segmentRotation + randomOffset;

    // No need to manually add a class here as we're using Angular's binding [class.spinning]
    ...
    if (typeof winningIndex === 'number' && winningIndex > 0) {
      ...
      if (prizeIndex !== -1) {
        ...
        if (
          ...
          numericPrizes[0].difference <= winningIndex * 0.2
        ) {
          // Use closest match if within 20% of winning value
      ...
      // No winning index provided, use random
      chosenIndex = Math.floor(Math.random() * this.segments.length);
    }
  ...
  ngOnDestroy(): void {
    ...
    // Cancel any running animation frame to avoid callbacks after destroy
    if ((this as any).activeAnimationFrame) {
      cancelAnimationFrame((this as any).activeAnimationFrame);
      (this as any).activeAnimationFrame = null;
    }
  }
    ...
    this._apiCall.PostCallWithToken(payload, 'Spinner/CustomerBet').subscribe(
      (response) => {
        if (response.responseCode === 200) {
          ...
          // Find the winning amount in segments or accept an index from API
          const apiValue = Number(this.customerBetWinningAmount);
          const prizeToFind = '$' + apiValue;
          let prizeIndex = this.segments.findIndex(
            (segment) => segment.prize === prizeToFind
          );

          // If not found by prize string, and apiValue looks like a valid 0-based index, use it
          if (prizeIndex === -1 && Number.isInteger(apiValue)) {
            if (apiValue >= 0 && apiValue < this.segments.length) {
              prizeIndex = apiValue;
            }
          }

          console.log(
            'Spinner API returned:',
          ...
          if (prizeIndex !== -1) {
            // Exact match found - if wheel is currently spinning, smoothly stop at that index
            if (this.isSpinning || this.isWheelSpinning) {
              this.smoothStopAt(prizeIndex);
            } else {
              this.testSpin(this.customerBetWinningAmount);
            }
          } else {
            // No exact match, find closest or use target function
            const hasCloseMatch = this.findClosestPrizeMatch(
              this.customerBetWinningAmount
            );

            if (!hasCloseMatch) {
              this.completeSpinToIndex(1);
              Swal.fire({
                icon: 'warning',
                title: 'Try Again',
  ...
  private completeSpinToTarget(winningAmount: number): void {
    ...
    const spinnerContainer = document.querySelector('.wheel-with-segments');
    const segmentAngle = this.segmentAngle;
    const currentPointerAngle = ((-this.rotation % 360) + 360) % 360;
    const sliceStart = chosenIndex * segmentAngle;
    const sliceEnd = sliceStart + segmentAngle;
  ...
  /**
   * Animate spinner to a specific index or a random index when not provided.
   * @param targetIndex optional index to land on (0-based). If omitted, a random index is used.
   */
  private completeSpinToIndex(targetIndex?: number): void {
    // const targetIndex = 3;
    if (this.currentSpinInterval) {
      clearInterval(this.currentSpinInterval);
      this.currentSpinInterval = null;
    ...
    const chosenIndex =
      typeof targetIndex === 'number' &&
        targetIndex >= 0 &&
        targetIndex < this.segments.length
    ...
    const settleOffset =
      typeof targetIndex === 'number' ? 8 : 8 + Math.random() * 10;
    ...
    const animate = (currentTime: number) => {
      ...
      if (t >= 1) {
        this.rotation = finalTargetRotation;
        // clear tracked animation frame id
        (this as any).activeAnimationFrame = null;
        this.completeRandomSpinAnimation(chosenIndex, spinnerContainer);
        return;
      }

      this.cdr.detectChanges();
      (this as any).activeAnimationFrame = requestAnimationFrame(animate);
    };

    // Start animation and track frame id so it can be cancelled on destroy
    (this as any).activeAnimationFrame = requestAnimationFrame(animate);
  ...
  /**
   * Test helper: instantly stop the spinner and land on the provided 0-based index.
   * This bypasses the smooth animation path and forces the final rotation so you can
   * validate behavior deterministically during debugging.
   */
  public stopAt(index: number): void {
    if (!this.segments || this.segments.length === 0) return;

    const chosenIndex = Math.max(
      0,
      Math.min(Math.floor(index), this.segments.length - 1)
    );

    // Clear any running intervals or animation frames
    if (this.currentSpinInterval) {
      clearInterval(this.currentSpinInterval);
      this.currentSpinInterval = null;
    }
    ...
    // Compute final rotation that places pointer on chosenIndex
    const segmentAngle = this.segmentAngle;
    const currentPointerAngle = ((-this.rotation % 360) + 360) % 360;
    const sliceStart = chosenIndex * segmentAngle;
    const desiredCenterAngle = sliceStart + segmentAngle / 2;
    const settleOffset = 8; // small deterministic offset for tests
    ...
    // Apply final rotation immediately
    this.rotation = finalTargetRotation;
    ...
    if (wheelEl) {
      wheelEl.style.transform = `rotate(${finalTargetRotation}deg)`;
    }
  ...
  /**
   * Smoothly stop the spinner while it's spinning and animate to the requested index.
   * This attempts to read the current rotation (from this.rotation or the DOM) and then
   * invokes the smoother RAF animation path to land on targetIndex.
   */
  public smoothStopAt(targetIndex: number): void {
    if (!this.segments || this.segments.length === 0) return;

    const chosenIndex = Math.max(
      0,
      Math.min(Math.floor(targetIndex), this.segments.length - 1)
    );

    // Clear any current interval (interval-based spin) and stop wheel CSS animation
    if (this.currentSpinInterval) {
      clearInterval(this.currentSpinInterval);
      this.currentSpinInterval = null;
    }
    ...
    // If wheel element has a computed transform from CSS animation, try to read it
    if (wheelEl) {
      ...
      if (transform && transform !== 'none') {
        ...
        if (match && match[1]) {
          ...
          if (parts.length >= 6 && !isNaN(parts[0]) && !isNaN(parts[1])) {
            // angle in radians = atan2(b, a)
            const a = parts[0];
            const b = parts[1];
      ...
      // Remove CSS spinning class to avoid conflicting animations
      wheelEl.classList.remove('spinning');
    }

    // Make sure pointer events are re-enabled for user interactions
    document.body.style.pointerEvents = 'none';

    // Start the smooth RAF-based animation to the chosen index
    this.completeSpinToIndex(chosenIndex);
  }
